...
def generate_wind_profile(seed=None):
    ...
    wind_profile = np.clip(np.random.normal(capacity_factor, 0.15, 8760), 0, 1) * 200  # Adjusted for peak 200 MW
    return wind_profile
...
def generate_pv_profile(seed=None):
    ...
    pv_profile = np.tile(daily_pattern, 365) * 200  # Adjusted for peak 200 MW
    return pv_profile


# Energy Storage System class
class EnergyStorageSystem:
    def __init__(self, duration=2, power=200, efficiency=0.81):
...
def calculate_reliability_metrics(duration=2):
    ...
    for year in range(Yr):
        wind_profile = generate_wind_profile(seed=year)
        pv_profile = generate_pv_profile(seed=year)
...
calculate_reliability_metrics(duration=2)